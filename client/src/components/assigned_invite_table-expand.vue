...
export default {
      ...
      methods: {
        ...
        sendEmail (params) {
          ...
          axios({
            method: 'post',
            url: config.baseUrl + '/vmailmicro/sendEmail',
                  ...
                  if (err.response.status === 401) {
                    ...
                    self.$Notice.error(
                      {duration: 0,
                        ...
                        desc: err.response.data.message + '. Please <a href="' + config.flowzDashboardUrl + '/subscription-list" target="_blank">Subscribe</a>'}
                                )
                  } else {
                    self.$Notice.error({
                      title: err.response.data.name,
