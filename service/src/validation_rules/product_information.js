 ...
 const ProductInformationRules = [
  {
    ...
    },
    {
        ...
        qryObject: (matrix_price)=>{ let regExp = new RegExp(/^\d{0,8}(\.\d{0,4})?$/); if (matrix_price == undefined) { return true } else if(matrix_price !== '' && !regExp.test(matrix_price)) { return true } else { return false } },
   ...
   },
    {
        ...
        qryObject: (matrix_frieght)=>{ let regExp = new RegExp(/^\d{0,8}(\.\d{0,4})?$/); if (matrix_frieght == undefined) { return true; } else if(matrix_frieght !== '' && !regExp.test(matrix_frieght)) { return true; } else { return false; } },
   ...
   },
    {
        ...
        qryMongo : {$and: [{ "vat": { $exists: true, $ne: "" } },
         { "vat" : {$not: /^\d{0,8}(\.\d{0,4})?$/ }}]},
        qryObject: (vat)=>{ let regExp = new RegExp(/^\d{0,8}(\.\d{0,4})?$/); if (vat == undefined) { return true } else if(vat !== '' && !regExp.test(vat)) { return true; } else { return false; } },
   ...
   },
   {
       ...
       qryObject: (packaging_charges)=>{ let regExp = new RegExp(/^\d{0,8}(\.\d{0,4})?$/); if (packaging_charges == undefined) { return true; } else if(packaging_charges !== '' && !regExp.test(packaging_charges)) { return true; } else { return false; } },
  ...
  },
  {
    ...
    qryObject: (video_url)=>{ let regExp = new RegExp(/https?:\/\/w{0,3}\w*?\.(\w*?\.)?\w{2,3}\S*|www\.(\w*?\.)?\w*?\.\w{2,3}\S*|(\w*?\.)?\w*?\.\w{2,3}[\/\?]\S*/); if (video_url == undefined) { return true; } else if( video_url !== '' && !regExp.test(video_url)) { return true; } else { return false; } },
...
},
{
    ...
    qryObject: (distributor_central_url)=>{ let regExp = new RegExp(/https?:\/\/w{0,3}\w*?\.(\w*?\.)?\w{2,3}\S*|www\.(\w*?\.)?\w*?\.\w{2,3}\S*|(\w*?\.)?\w*?\.\w{2,3}[\/\?]\S*/); if (distributor_central_url == undefined) { return true; } else if(distributor_central_url !== '' && !regExp.test(distributor_central_url)) { return true; } else { return false; } },
